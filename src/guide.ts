...
export function creatorGuideText(status: CreatorSetupStatus): string {
  ...
  const lines: string[] = [
    ...
    "- 新建一集用 azu_create_content (或 mz_create_content)；文件夹名乱了用 azu_organize_library (或 mz_organize_library)，先预览、用户确认后再 apply=true。它只改名，不删文件。",
    "- 工作台自己的状态（绑定、阶段、发布标记）在 overlay.json，不是正文，不要手改。",
    "",
    "## 脚本与人设",
    "- 每集的口播脚本在 script.md，直接用文件工具读写。",
    scriptRules === undefined
      ? "- 当前没有配置脚本规则（人设）。用户第一次让你写或改脚本时，先主动问清语气、结构、禁忌和目标观众，再用 azu_script_rules (或 mz_script_rules) 存下来，之后每次写脚本都遵循。"
      : "- 已配置脚本规则（人设），写或改 script.md 前先用 azu_script_rules (或 mz_script_rules) 读取并严格遵循；用户提出新的长期偏好时，把它合并进规则再保存。",
    "",
    "## 字幕",
    "- 成片落盘后：azu_generate_subtitles (或 mz_generate_subtitles) 转录、自动校对、排版，完成后打开预览；用户在预览里确认专有名词后，再用 azu_burn_subtitles (或 mz_burn_subtitles) 烧录。预览前不要烧录。",
    "- azu_generate_subtitles (或 mz_generate_subtitles) 是长任务，调用后立即返回。完成看 subtitle-transcript.json / subtitle-manifest.json，不要等 *_subtitled.mp4，也不要把启动说成完成。",
    ...subtitleLines(status),
    "",
    "## 封面",
    "- azu_generate_cover (或 mz_generate_cover) 前先按 oil-cover 从脚本或字幕提炼封面主标题，通过 title 传入；不要把文件夹名直接当封面结论。生成后请用户核对标题文字和错别字，不对就再生成。",
    ...coverLines(capabilities),
    "",
    "## 制作工程与成片",
    "- 录制、剪辑和导出可以使用任意制作工具。用 azu_update_content (或 mz_update_content) 的 productionProjectPath 绑定对应一集的工程或素材目录；用 azu_open_production_project_folder (或 mz_open_production_project_folder) 打开所在目录。绑定不限制文件格式，也不要求 Screen Studio。",
    "- azu_wait_export (或 mz_wait_export) 会等待这一集文件夹中稳定的 MP4/MOV 成片，不依赖具体剪辑软件。它是长任务，启动后轮询文件或 getContent，不要把启动说成完成。",
