...
内容管理已提供本地小闭环：点击“新建内容”，编辑并保存稿件，在“账号管理”登记一个或多个账号，再为内容选择目标账号、手动登记发布链接与时间。账号登记与这些记录不依赖 `video-publisher`。B站支持扫码连接和真实身份检查，见[连接说明](docs/bilibili-connection.md)；其他平台的登记仍不代表登录成功。详见[本地内容管理](docs/content-management.md)。

原有平台连接、自动发布和数据回收保留在内容详情的“高级制作与自动发布”中，仍需要 `video-publisher`、浏览器与相应账号能力。手动记录不会自动触发平台操作。
