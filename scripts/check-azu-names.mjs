...
const oldDataFiles=new Set(['src/config.ts','src/catalog.ts','tests/config.test.ts','tests/dataDirectory.test.ts','tests/catalog.test.ts','tests/previewServers.test.ts','tests/collectSpaces.test.ts','docs/mz-naming.md','docs/azu-naming.md']);
...
const interfaceHistoryFiles=new Set(['docs/mz-naming.md','docs/azu-naming.md','tests/mzNaming.test.ts','tests/azuNaming.test.ts']);
...
const ignored=new Set(['scripts/check-mz-names.mjs','scripts/check-azu-names.mjs','tests/mzNamingGuard.test.ts','tests/azuNamingGuard.test.ts']);
...
if(failures.length){console.error(failures.join('\n'));process.exitCode=1;}else console.log('AZU naming check passed: only documented upstream and legacy-data exceptions remain.');
